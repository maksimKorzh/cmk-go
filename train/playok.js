    ...
    case 'start':
      ...
      setTimeout(function() {
        if (!activeGame) {
          ...
          if (cmkgoSide == 0) {
            cmkgo.stdin.write('clear_board\n');
            cmkgo.stdin.write('genmove B\n');
            cmkgo.stdin.write('showboard\n');
          }
        }
      }, 5000);
      break;
...
function connect() {
  ...
  socket.on('open', function () {
    const initialMessage = JSON.stringify({
      "i":[1721],
      "s":[
        "+442266185138966324|3555651168|18165991", // guest tgb6968g
        "en",
        "b",
        "",
    ...
    if (response.i[0] == 91 && response.i[1] == TABLE && joinedTable) { // load game
      ...
      if (moves != undefined) {
        for (let move of moves) {
          let color = side ? 'W' : 'B';
          if (move == '-') cmkgo.stdin.write('play ' + color + ' pass\n');
          else cmkgo.stdin.write('play ' + color + ' ' + move.replace('-', '').split(' ')[0] + '\n');
          side ^= 1;
        }
      }
      cmkgo.stdin.write('showboard\n');
      ...
      if (move != undefined) {
        if (side == cmkgoSide ^ 1) {
          ...
          if (move == '-') {
            ...
            cmkgo.stdin.write('play ' + color + ' pass\n');
          } else {
            console.log('playok: received move ' + move.split('-')[0].toUpperCase() + move.split('-')[1]);
            cmkgo.stdin.write('play ' + color + ' ' + move.replace('-', '').split(' ')[0] + '\n');
            cmkgo.stdin.write('showboard\n');
          }
        }
        side ^= 1;
        if (side == cmkgoSide) {
          color = cmkgoSide ? 'W' : 'B';
          cmkgo.stdin.write('genmove ' + color + '\n');
          cmkgo.stdin.write('showboard\n');
        }
      }
    }
...
cmkgo.stdout.on('data', (data) => {
  ...
  if (isMove) {
    ...
    let sq = row * 19 + col;
    setTimeout(function() {
      console.log('cmkgo: generated move', move);
      let message = JSON.stringify({"i": [92, TABLE, 0, sq, 0]});
      socket.send(message);
    }, 1000);
  } else if (response.includes('PASS')) {
    console.log('cmkgo: PASS');
    message(socket, 'pass', TABLE);
  } else if (response.includes('resign')) {
    console.log('cmkgo: RESIGN');
    message(socket, 'resign', TABLE);
  } else if (response.includes('A B C D E F')) {
  } else {
    if (data.toString().includes('illegal')) {
      console.log('cmkgo: something went wrong')
    }
  }
});

cmkgo.on('exit', (code, signal) => {
  console.error('cmkgo: exited');
});

cmkgo.on('error', (err) => {
  console.error('cmkgo:', err);
});

cmkgo.stderr.on('data', (data) => {
  console.log('cmkgo:', data.toString());
});

process.on('SIGINT', function() { // Ctrl-C: force resign, Ctrl-\ to quit (linux)
  if (side == cmkgoSide) message(socket, 'resign', TABLE);
});

setInterval(function() {
  // Debug
  //let timestamp = new Date().toJSON();
  //console.log(timestamp + ' cmkgo ' + cmkgo.stdin.writable + ', socket ' + socket.readyState + ', side ' + side + ', cmkgoSide ' + cmkgoSide + ', table ' + TABLE + ', joinedGame ' + joinedTable + ', activeGame ' + activeGame);
